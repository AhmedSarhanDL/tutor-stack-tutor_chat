...
def test_answer_question(client):
    ...
    assert data["answer"] == "This is a test response for the automated PR. In production, this would be a real tutored answer."
...
def test_answer_validation(client):
    ...
    # Test invalid JSON
    response = client.post(f"{client.base_url}/answer", data="invalid json", headers={"Content-Type": "application/json"})
    assert response.status_code == 422
...
def test_long_question(client):
    ...
    long_question = "What is the relationship between mass, acceleration, and force according to Newton's second law of motion, and how does this apply to real-world scenarios like car crashes and rocket propulsion?"
